...
The workspace's `.cursor/mcp.json` launches the local `dist/stdio.js` using `node` and Cursor's `${workspaceFolder}` interpolation. Ensure the host resolves Node 24, then reload the plan-viz server in Cursor after building to activate these changes. For release-based installations, use the [v0.1.1 tarball](https://github.com/NGA-TRAN/plan_viz_mcp/releases/tag/v0.1.1), which includes these fixes; v0.1.0 does not. Codex needs its own [MCP configuration](../README.md#codex-setup).
